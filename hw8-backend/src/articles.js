...
const uploadTextAndImage = require('./uploadCloudinary').uploadTextAndImage
...
const fetchArticle = (req, res) => {
    ...
    if (id != undefined) {
        ...
        Profile.find({username: req.username}).exec(function(err, profile){
            ...
            Article.find({author: {$in: following}}).sort('-date').limit(10).exec(function(err, articles){
...
const postArticle = (req, res) => {
    ...
    if(!req.textMsg){
        ...
        const article = new Article({author: req.username, img: req.fileurl,text: req.textMsg,date: new Date(),comments: []})
...
module.exports = (app) => {
    app.post('/article',uploadTextAndImage('articleImg'), postArticle)
